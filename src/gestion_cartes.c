...
void lister_pioche(Pioche* entete, char* nom_carte)
{
        int i, j = 0;
        struct dirent *lecture;
        char* lec;
        DIR *rep;
        Pioche* curseur_anc = entete;
//      Pioche* curseur_nouv = entete;
        Pioche* curseur_nouv = NULL;// = (Pioche*)malloc(sizeof(Pioche));

        rep = opendir("./images/" );
        while ((lecture = readdir(rep))) {
                lec = lecture->d_name;
                lec += strlen(lec) - 4;
                if (!strncmp(lecture->d_name, nom_carte, strlen(nom_carte)) && !strcmp(lec, ".bmp")) {
//                      printf("%s\n", lecture->d_name);
//printf("début if\n");
                        // Nouveaux attributs de la nouvelle carte
                        curseur_nouv = (Pioche*)malloc(sizeof(Pioche));
//printf("Avant strpcy\n");

                        curseur_nouv->nom_carte = (char*)malloc(strlen(lecture->d_name));
                        strcpy(curseur_nouv->nom_carte, lecture->d_name);
                //      curseur_nouv->nom_carte = realloc(curseur_nouv->nom_carte, strlen(curseur_nouv->nom_carte) + 1);

//printf("Nom nouv: %s\n", curseur_nouv->nom_carte);
                        curseur_nouv->carte_precedente = curseur_anc;
                        curseur_nouv->carte_suivante = entete;
//                      printf("%s\n", curseur_nouv->nom_carte);

                        // Réfère sur la nouvelle carte (suivante de la précédente)
                        curseur_anc->carte_suivante = curseur_nouv;
                        curseur_anc = curseur_nouv;
